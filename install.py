...
MEMORY_PROTOCOL_LINES = [  # v6.2.1
    ...
    "",
    "THREE-TIER MEMORY SYSTEM -- each tier has a different cost and scope:",
    "",
    "  TIER 1: USER PROFILE (zero cost, always present)",
    "    Broad universal preferences baked directly into this system prompt.",
    "    Applies to EVERY project regardless of type.",
    "    Updated via profile_update(). Capped at 12 items.",
    "    Examples: communication style, display preference, language, accessibility needs.",
    "",
    "  TIER 2: DOMAIN PREFERENCE NODES (loaded when domain is detected)",
    "    One node per domain type, tagged ['preference', 'domain-pref', '<domain>'].",
    "    Only loads when the relevant domain is detected in the current project.",
    "    Multiple domains? Multiple nodes load. Server + HTML? Both load. EXE? Neither.",
    "    Domains: html | server | python | exe | data | cli | notebook | mobile",
    "",
    "  TIER 3: GRAPH MEMORY (on demand)",
    "    Projects, decisions, session history, relationships, named tools.",
    "    Loaded via query() then expand(). Never loaded unless relevant.",
    "",
    ...
    "                    similarity > 0.87. On success: auto-links to similar nodes.",
    "  merge()         → combines two nodes. Unions content. Transfers all edges. Deletes absorbed.",
    "  split()         → divides one node into two linked nodes. Both inherit original edges.",
    "  update()        → edits summary/content/tags/importance in place. ID and edges NEVER touched.",
    "  query()         → hybrid scoring: 65% text + 35% importance. High-importance nodes surface first.",
    "  find_similar()  → given a node, discovers related nodes NOT yet linked. For gap-filling.",
    "  profile_update() → patches USER PROFILE block in this system prompt. Broad universal prefs only.",
    "  profile_get()   → reads current USER PROFILE block.",
    "",
    ...
    "",
    "### DOMAIN PREFERENCE NODES (Tier 2) — how they work:",
    "  Before generating anything for a project, detect all domains present in the message.",
    "  Domains: html | server | python | exe | data | cli | notebook | mobile",
    "",
    "  Signal words per domain (not exhaustive):",
    "    html:     webpage, frontend, CSS, HTML, Tailwind, React, UI, dark mode on a page",
    "    server:   FastAPI, Flask, API, endpoint, backend, route, REST, Django, Express",
    "    python:   script, .py, automation, library, package, uv, pip",
    "    exe:      desktop app, installer, .exe, WPF, Electron, compiled, winforms",
    "    data:     database, SQL, BigQuery, SQLite, pipeline, ETL, analytics, dataframe",
    "    cli:      command line, terminal, argparse, click, shell script, bash",
    "    notebook: Jupyter, .ipynb, data science, matplotlib, pandas exploration",
    "    mobile:   iOS, Android, Flutter, React Native, Swift, Kotlin",
    "",
    "  For EACH detected domain, fire in parallel:",
    "    list(filter_tags=['preference', 'domain-pref', '<domain>'], top_k=1)",
    "  If a node exists: expand it. Apply its preferences before generating anything.",
    "  If no node exists: proceed with Tier 1 profile defaults.",
    "",
    "  Multi-domain example: 'Build a FastAPI server that serves an HTML dashboard'",
    "    → detect: server + html",
    "    → fire: list(filter_tags=['preference','domain-pref','server'], top_k=1)",
    "            list(filter_tags=['preference','domain-pref','html'],   top_k=1)  [parallel]",
    "    → expand both if found. Apply both. EXE node: never touched.",
    "",
    "  When user states a domain-specific preference (e.g. 'I always use Tailwind for HTML'):",
    "    1. Check if a node tagged ['preference','domain-pref','html'] already exists.",
    "       (query or list with filter_tags, top_k=1)",
    "    2. EXISTS → update(memory_id=<id>, content='...', append=True)",
    "    3. NEW    → save(summary='HTML prefs: Tailwind, dark mode, WCAG AA',",
    "                     content='...', tags=['preference','domain-pref','html'], importance=8)",
    "  One node per domain max. Update it, don't create duplicates.",
    "",
    "### WHAT GOES WHERE — decision tree:",
    "  Applies to ALL projects, communication style, display?    → Tier 1: profile_update()",
    "  Specific to a project type (HTML/server/EXE/etc)?         → Tier 2: domain node",
    "  Project-specific detail, decision, session, named person? → Tier 3: save() to graph",
    ...
    "  summary:  short, searchable, self-identifying (80-150 chars).",
    ...
    "            The 'what is this really about' layer. Recommended for any node that may grow.",
    "    GOOD:  'DataSync syncs inventory between two warehouses. Node.js + PostgreSQL 14.",
    "            Auth via JWT (WIP). Known bug: batch > 500 items fails silently. AWS us-east-1.'",
    "",
    "  content:  full rich detail. Paginated via content_limit/content_offset.",
    "    BAD:   same as summary or abstract (defeats the purpose)",
    "    GOOD:  everything someone would need to fully understand this node.",
    "",
    ...
    "PROFILE_UPDATE: universal_constructor(action='call', tool_name='cyber_memory', tool_args={\"action\": \"profile_update\", \"summary\": \"display\", \"content\": \"dark mode always\"})",
    "PROFILE_GET:    universal_constructor(action='call', tool_name='cyber_memory', tool_args={\"action\": \"profile_get\"})",
    "DOMAIN PREF QUERY: universal_constructor(action='call', tool_name='cyber_memory', tool_args={\"action\": \"list\", \"filter_tags\": [\"preference\", \"domain-pref\", \"html\"], \"top_k\": 1})",
    "DOMAIN PREF SAVE:  universal_constructor(action='call', tool_name='cyber_memory', tool_args={\"action\": \"save\", \"summary\": \"HTML prefs: Tailwind, dark mode, WCAG AA\", \"content\": \"...\", \"tags\": [\"preference\", \"domain-pref\", \"html\"], \"importance\": 8})",
    ...
    "Cap: 12 items. Prefer updating existing entries over adding new ones.",
